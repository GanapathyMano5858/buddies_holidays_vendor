...
class TripAdvanceReportModel
{
  ...
  public function TripadvanceReport($GetDatas)
  {
    ...
    $ClientList = $this->pdo->prepare('select ve.trip_went_not,ve.va_transporter_id, ve.va_id,b.details,b.cancel_status,b.arrivedornot,b.id_client, b.ref_no,b.client_name,b.start_from,b.end_to,b.client_arrival_name,b.client_departure_name,b.transporter_id as curt_transporter_id,b.confirmedprice, b.rejected_vendors,b.remarks,tr.*, e.t_id,e.reference_no,e.trans_vendors,e.transporter_name,e.dest_name,e.bank_access_id ,e.trans_type  from ps_vehicle_allotment as ve left join ps_client as b on (ve.client_table_id=b.id_client) left join  ps_trip_advance as tr on(ve.va_id =tr.ta_va_id) left join ps_transporter as e on (e.t_id=ve.va_transporter_id )  where ' . $where . ' and b.status=0  and b.transporter_id!="" and ' . $vendor . '   order by ' . $oderby . '');
  ...
  public function ViewDetails($GetDatas)
  {
    ...
    $heading = '<button type="button" class="close clst btn btn-outline-danger btn-sm" data-dismiss="modal"><span aria-hidden="true"><i class="fa-solid fa-x"></i></span><span class="sr-only">Close</span></button><h4 class="modal-title text-center" id="myModalLabel">' . $headings . '</h4>';
  ...
  <hr class="m-0 ">
    <h3 class="clienrdeth3 m-0" style="text-align: center; font-size:18px"> Client Details</h3>
    <hr class="m-0 pb-2">
    <div class="d-flex flex-column gap-2" style="font-size:13px;">
    <div class="d-flex justify-content-between align-items-center">
      <label class="col-xs-6 pdb">BH Ref No </label>
      <span class="col-xs-6 pdb margB"><b>' . $clientList['ref_no'] . '</b></span>

    </div>
    
    <div class="d-flex justify-content-between align-items-center">
      <label class="col-xs-6 pdb">Client Name</label>
      <span class="col-xs-6 pdb margB"><b>' . $clientList['client_name'] . '</b></span>
    </div>
    ...
    <div class="table-responsive viewtable">
      ...
      <thead>
        <tr class="tabhead table-secondary">
          <th style="font-size:11px" class="text-nowrap">Trip Advance Request</th>
          <th style="font-size:11px" class="text-nowrap">Trip Advance Paid</th>
          <th style="font-size:11px" class="text-nowrap">Approval Status</th>
          <th style="font-size:11px" class="text-nowrap">Paid Status</th>
          <th style="font-size:11px" class="text-nowrap">Utr No & Date</th>
          <th style="font-size:11px" class="text-nowrap">Remark</th>
    ...
    foreach ($tripAdvace_paid as $valuess) {
      echo '<tr class="align-middle text-center"> <td>' . $valuess['request'] . '</td> <td>' . $valuess['paid'] . '</td> <td>' . $valuess['approval_status'] . '</td> <td>' . $valuess['paid_status'] . '</td>';
      if ($valuess['paid_status'] == 'Partially Paid' || $valuess['paid_status'] == 'PAID') {
        echo '<td>' . $valuess['utr'] . '<br>' . $valuess['paid_date'] . '</td>';
      } else {
        echo '<td>-</td>';
      }
